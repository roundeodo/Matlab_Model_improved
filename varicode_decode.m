...
varicode_table = [
    ...
    % 2. ASCII index (0~127)
    ascii_table = char(0:127); % 

    % 3. transfer bits to string
    bit_str = char(bits + '0')';

    % 4. divide the string by "00"
    varicode_words = strsplit(bit_str, "00");
    varicode_words = varicode_words(2:end-1);

    % 5. match with the LUT
    message_chars = [];
    for i = 1:length(varicode_words)
        word = varicode_words{i};
        idx = find(varicode_table == word, 1); 
        if ~isempty(idx)
            message_chars(end + 1) = ascii_table(idx);  
        else
            message_chars(end + 1) = ' ';  
        end
    end

    % 6. recover the original message
